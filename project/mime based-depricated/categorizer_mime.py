...
class destination_folder(folder):
    ...
    def __init__(
        self,
        input_dir=input(f"Enter the destination folder for the categories.\n \
                     deffault:{os.getcwd()}:"),
    ):
        super().__init__()
...
def logger(*args, func=None, msg):
    ...
    log = (jdate("Y/m/d H:i:s") + "\t" + str(msg) + "\t" + output + "\n" +
...
if __name__ == "__main__":
    with open(os.path.join(os.getcwd(), "log" + jdate("Y-m-d-H-i-s") + ".txt"),
              "a") as log_file:
