...
class FontFile(MyFile, Font):
    ...
    def revert_font_name(self):  # not testes Yet!
        ...
        logger.info(
            f"Font reverted to its original file_name: {self.file_name}")
...
class ArchiveFile(MyFile):
    ...
    def test_archive(self):
        try:
            ...
            logger.error(f"Error. Testing {self.path} Failed with error {e}")
            return False
    ...
    def check_integerity(self):
        ...
        # not tested for documents & apks
            ...
            logger.debug(
                "Integerity check not implemented for This file type! Skipping"
            )
            return True
