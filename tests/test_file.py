...
print(testcase_file.parent_dir, PurePath('/home/hadi/Documents/GitHub/test center/CataFile/test_cases'))

def test_get_parent_folder(): # In logs it represnts only  one dot! why and how?!
    """Check parent folder representation"""
    return testcase_file.parent_dir == PurePath('/home/hadi/Documents/GitHub/test center/CataFile/test_cases')  # full path includes name no need 2 fullname
...
def test_file_date_time_function(): # Passed!
    return testcase_file.file_date_time() == os.path.getatime(testcase_file.full_path), os.path.getctime(testcase_file.full_path), os.path.getmtime(testcase_file.full_path)
...
def test_file_detection_with_fleep():
    return fleep.get(testcase_file.file_object.read(2048)) == testcase_file.fleep_detect()
...
def test_move_function(): # Has problems with not existing directory, same is true about copy function
